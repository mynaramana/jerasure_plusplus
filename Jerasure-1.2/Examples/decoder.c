...
enum Coding_Technique {Reed_Sol_Van, Reed_Sol_R6_Op, Cauchy_Orig, Cauchy_Good, Liberation, Blaum_Roth, Liber8tion, RDP, EVENODD, No_Coding, PiggybackRS, LRC_RS};
...
int main (int argc, char **argv) {
  ...
  if(tech == PiggybackRS){
    k_ = 2*k_;
    m_ = 2*m_;
  }else if(tech == LRC_RS)m_ = m+2;
   
  /* Allocate memory */
  erased = (int *)malloc(sizeof(int)*(k_+m_));
  for (i = 0; i < k_+m_; i++)
    erased[i] = 0;
  erasures = (int *)malloc(sizeof(int)*(k_+m_));

  data = (char **)malloc(sizeof(char *)*k_);
  coding = (char **)malloc(sizeof(char *)*m_);
  if (buffersize != origsize) {
    for (i = 0; i < k_; i++) {
      data[i] = (char *)malloc(sizeof(char)*(buffersize/k_));
      memset(data[i], 0, sizeof(char)*(buffersize/k_));
    }
    for (i = 0; i < m_; i++) {
      coding[i] = (char *)malloc(sizeof(char)*(buffersize/k_));
      memset(coding[i], 0, sizeof(char)*(buffersize/k_));
    }
    blocksize = buffersize/k_;
  }
  ...
  case PiggybackRS:
    ...
    break;
  case LRC_RS:
    matrix = lrc_rs_vandermonde_coding_matrix(k,m,w,2);
    break;
  }
  gettimeofday(&t4, &tz);
  tsec = 0.0;
  ...
  while (n <= readins) {
	...
	numerased++;
	if(k != k_){//for piggyback 1
	  erased[i-1+k] = 1;
	  erasures[numerased] = i-1+k;
	  numerased++;
	}
      }
      else {
	if (buffersize == origsize) {
	  ...
	  if(k_ > k){
	    fread(data[i-1], sizeof(char), blocksize/2, fp);
	    data[i-1+k] = (char*)malloc(sizeof(char*)*blocksize);
	    fread(data[i-1+k], sizeof(char), blocksize/2, fp);
	  }else{
	  ...
	  fseek(fp, (n-1)*buffersize/k, SEEK_SET);			
	  fread(data[i-1], sizeof(char), buffersize/k_, fp);
	  if(tech == PiggybackRS){
	    fread(data[i-1+k], sizeof(char), buffersize/k_, fp);
	  }
	}
	fclose(fp);
      }
    ...
    for (i = 1; i <= m; i++) {
      ...
      if (fp == NULL) {
	erased[k_+(i-1)] = 1;
	erasures[numerased] = k_+i-1;
	numerased++;
	if(k_ > k){
	  erased[k_+m+i-1] = 1;
	  erasures[numerased] = k_ + m + i - 1; 
	  numerased++;
	}
	//printf("%s failed\n", fname);
      }
	...
	if (buffersize == origsize) {
	  ...
	  if(tech == PiggybackRS){
	    fread(coding[i-1], sizeof(char), blocksize/2, fp);
	    coding[i-1+m] = (char*)malloc(sizeof(char)*blocksize);
	    fread(coding[i-1+m], sizeof(char), blocksize/2, fp);
	  }else{
	    fread(coding[i-1], sizeof(char), blocksize, fp);
	  }
	}
	else {
	  fseek(fp, buffersize*(n-1)/k, SEEK_SET);
	  fread(coding[i-1], sizeof(char), blocksize, fp);
	  if(tech == PiggybackRS){
	    fread(coding[i-1+m], sizeof(char), blocksize, fp);
	  }
	}	
	fclose(fp);
      }
    ...
    if(tech == LRC_RS){
      for(i=m+1;i<=m_;i++){
	sprintf(fname, "%s/Coding/%s_l%0*d%s", curdir, cs1, md, i-m, cs2);
	fp = fopen(fname, "rb");
	if (fp == NULL) {
	  erased[k_+(i-1)] = 1;
	  erasures[numerased] = k_+i-1;
	...
	memset(data[erasures[i]], 0, sizeof(char)*blocksize);
      }
      else {
	if (n == 1)
	  coding[erasures[i]-k_] = (char *)malloc(sizeof(char)*blocksize);
	memset(coding[erasures[i]-k_], 0, sizeof(char)*blocksize);
      }
    }
    ...
    //printf("GOING TO START DECODING\n");
    /* Choose proper decoding method */
    if (tech == Reed_Sol_Van || tech == Reed_Sol_R6_Op) {
      i = jerasure_matrix_decode(k_, m_, w, matrix, 1, erasures, data, coding, blocksize);
      //i = 0;
    ...
    else if(tech == PiggybackRS)
      {
	if(numerased == 2){
	  piggyback1_rs_decode(k, m, w, matrix, erasures, data, coding, buffersize/k_);
	  i = 0;
	}else if(numerased == 0)i=0;
	else{
	  i = -1;
	}
      }
      ...
      if(numerased == 1){
	lrc_rs_vandermode_decode(k,m,w, 2,matrix,erasures,data,coding,blocksize);
      }
      i = 0;
    }
    else {
    ...
    for (i = 0; i < k; i++) {
      if (total+buffersize/k <= origsize) {
	fwrite(data[i], sizeof(char), buffersize/k_, fp);
	total+= blocksize;
	if(tech == PiggybackRS){
	  fwrite(data[i+k], sizeof(char), buffersize/k_, fp);
	  total += blocksize;
	}
      }
      else {
